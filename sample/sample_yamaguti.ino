...
void setup() {
  Serial.begin(9600);
  ICS1.begin(1250000);//サーボの通信速度を1250000に設定しているためカッコ内を変える
  ICS3.begin(1250000);
  ICS4.begin(1250000);
  //回収装置セットアップ
  for(int i = 0; i < 5; i++){
    ...
    delay(1);
    krs_setposition(&C_servo[i], 45);//回収装置の初期角度は45度．後々で調整必要かも
    delay(1);
  }
  //前輪セットアップ
  for(int i = 0; i < 2; i++){
  ...
  delay(1);
  R_servo[2].setSpeed(30); //少しゆっくりにしてみた
  delay(1);
...
void loop() {
  //回収装置
  krs_setposition(&C_servo[4], 45); //一応入れている
  krs_setposition(&C_servo[3], 45); //セットアップでこの角度になっていいるはず
  delay(500);
